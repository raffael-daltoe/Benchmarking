...
// 1 => MRU , 0 => LRU
void CACHE::initialize_replacement() { ::last_used_cycles[this] = std::vector<bool>(NUM_SET * NUM_WAY); }
...
{   double eps = 1.0;
    if(!hit){
        ...
        if (random_value < eps){
            //assign as MRU
            ::last_used_cycles[this].at(set * NUM_WAY + way) = 1;
            
        }
        else{
            // assign as LRU
            
            ::last_used_cycles[this].at(set * NUM_WAY + way) = 0;
        }
    }

    else if(access_type{type} != access_type::WRITE && ::last_used_cycles[this].at(set * NUM_WAY + way) == 0){
