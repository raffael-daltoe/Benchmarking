...
emissary::emissary(CACHE* cache) : emissary(cache, cache->NUM_SET, cache->NUM_WAY) {}

emissary::emissary(CACHE* cache, long sets, long ways) : replacement(cache), NUM_WAY(ways), last_used_cycles(static_cast<std::size_t>(sets * ways), 0), priority_bits(static_cast<std::size_t>(sets * ways), 0) {}
          ...
          champsim::address full_addr, access_type type)
{
	...
	if(hp_count >= hp_threshold){
        	if (priority_bits[std::distance(begin_cycles, i)] == 1) { // If high priority
        	    if (last_used_cycles[std::distance(begin_cycles, i)] < min_cycle) {
        	        min_cycle = last_used_cycles[std::distance(begin_cycles, i)];
        	        victim_index = static_cast<uint32_t>(std::distance(begin_cycles, i));
        	    }
        	}
    	}
	else {
        	if (priority_bits[std::distance(begin_cycles, i)] == 0) { // If low priority
        	    if (last_used_cycles[std::distance(begin_cycles, i)] < min_cycle) {
        	        min_cycle = last_used_cycles[std::distance(begin_cycles, i)];
        	        victim_index = static_cast<uint32_t>(std::distance(begin_cycles, i));
        	    }
...
// random probability of 1/32 (take int having range from 0 to 31 when the value of random bit is 0 then condition is true)
    ...
    if (hit) {
        if (result && (hp_count < hp_threshold)) {
            priority_bits[set * NUM_WAY + way] = 1; // Cache hit and condition_x is true
			...
			//auto begin_cycles = std::begin(last_used_cycles[this]) + set * NUM_WAY;
		    ...
		    //Assigning high priority to the new cache hit line
                    priority_bits[set * NUM_WAY + way] = 1; // Cache hit and random_probability is true
		
        } 
	else {
            priority_bits[set * NUM_WAY + way] = 0; // Cache hit, and random probability is false
        }
	...
	if (access_type{type} != access_type::WRITE){
	   last_used_cycles.at((std::size_t)(set * NUM_WAY + way)) = cycle;
        ...
        if (result && (hp_count < hp_threshold)) {
            priority_bits[set * NUM_WAY + way] = 1; // Cache miss and random probability is true
            ...
            priority_bits[set * NUM_WAY + way] = 0; // Cache miss and random probability is false, or any other case, set low priority
        }
    }
